...
# --- Synthetic Event Classes for Web Scraping --- #
class SyntheticMessage:
    """一个模拟的 Telethon Message 对象，用于传递抓取到的文本。"""
    def __init__(self, text):
        self.text = text
        self.id = None  # 消息ID，对于处理链可能不是必需的
        self.sender_id = None # 发送者ID
        self.chat_id = None # 聊天ID
        self.grouped_id = None
        self.media = None
        self.entities = []

class SyntheticEvent:
    """一个模拟的 Telethon Event 对象。"""
    def __init__(self, text, client):
        self.message = SyntheticMessage(text)
        self.client = client
        # 其他可能被过滤器链访问的属性可以按需添加
        self.sender_id = None
        self.chat_id = None
