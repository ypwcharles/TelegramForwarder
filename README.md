...
将指定网页内容定期抓取并用 AI 总结后，自动发送到目标频道。目前内置了对 CoinMarketCap Community 的增量抓取能力，可按币种页面汇总近 24 小时的新动态。

### 启用功能
- 使用容器部署：Docker 镜像基于 Playwright 官方镜像构建，已包含 Chromium 运行环境，无需额外安装。
- 本地运行（可选）：
  - 安装依赖：`pip install -r requirements.txt`
  - 安装浏览器：`playwright install --with-deps`

### 创建任务
1. 在与机器人的私聊窗口发送命令：
   - `/webscrape` 或 `/ws`
2. 在弹出的菜单中点击「➕ 添加新任务」，按提示依次完成：
   - 任务名称：自定义
   - 币种名称：逗号分隔，例如 `bitcoin,ethereum,solana`
     - 程序会访问形如 `https://coinmarketcap.com/community/coins/{coin}/latest/` 的页面，请使用对应的币种路径名
   - 目标频道：发送频道链接或 ID
     - 发送使用机器人账号，需保证机器人对该频道有发言权限（建议设为管理员）
   - 定时（Cron）：可从快捷选项选择，也可「手动输入」完整表达式
3. 列表中可对任务进行：启用/禁用、编辑币种、编辑定时、设置频道、AI 设置、立即执行一次等操作。

### AI 设置
- 在任务设置中点击「AI 设置」：
  - 更改 AI 模型：来自 `config/ai_models.json` 的模型列表
  - 设置总结提示词：将抓取的多条内容作为输入，按你的提示词生成汇总文本
- .env 中需配置对应服务的 API Key（如 OpenAI、Claude、Gemini 等）。

### 调度与执行
- 调度时区：默认 `Asia/Shanghai`。
- 抓取窗口：默认仅处理近 24 小时内的帖子；重复帖子会自动去重，历史已处理记录会持久化，避免重复总结。
- 运行时会将每个币种的新动态汇总生成一段文本，并以机器人账号发送到目标频道。

### 注意事项
- 目标站点目前适配了 CoinMarketCap Community 的页面结构；若结构变更，抓取可能失效。
- 首次启用请先使用「▶️ 立即执行一次」验证流水线（抓取 → 总结 → 发送）。
- 容器运行环境已包含 Playwright 与浏览器；本地运行需提前安装浏览器。
...
网页抓取相关
/webscrape(/ws) - 管理网页抓取任务（添加/编辑/启用任务、AI 设置、手动触发）
