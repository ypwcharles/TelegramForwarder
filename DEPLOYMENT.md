...
```bash
git clone https://github.com/ypwcharles/TelegramForwarder.git
...
- `docker-compose.yml`: 用于本地开发和构建。
- `docker-compose-syn.yml`: **用于线上部署的模板**。
...
```bash
# 示例：将版本号更新为 v2.2.0
docker buildx build --platform linux/amd64 -t charlesyang10107/bstanewsforwarder:v2.2.0 --push .
```

**2. （本地）更新部署文件**

修改 `docker-compose-syn.yml` 文件，将 `image` 字段指向您刚刚推送的新版本号。

```yaml
# docker-compose-syn.yml
services:
  telegram-forwarder:
    image: charlesyang10107/bstanewsforwarder:v2.2.0 # <-- 更新这里的版本号
    ...
```
