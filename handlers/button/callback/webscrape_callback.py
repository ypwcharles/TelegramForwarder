...
from utils.common import get_bot_client, get_ai_settings_text
...
async def handle_webscrape_callback(event, data):
    ...
    # 无需任务ID的操作
    if action in ['ws_add_new', 'ws_back_to_list', 'ws_close', 'ws_cancel_add']:
    ...
    if action == 'ws_task': await handle_ws_task_settings(event, task_id)
...
async def handle_ws_edit_coins(event, task_id):
    ...
    await event.edit("请输入**币种名称** (多个用逗号 `,` 分隔):", buttons=[Button.inline("⬅️ 返回", f"ws_task:{task_id}")], parse_mode='markdown')
...
async def handle_ws_edit_schedule_menu(event, task_id):
    ...
    await event.edit("请选择或手动输入定时 (Cron表达式)：", buttons=buttons)
...
async def handle_ws_manual_schedule(event, task_id):
    ...
    await event.edit("请输入 Cron 表达式:", buttons=[Button.inline("⬅️ 返回", f"ws_task:{task_id}")], parse_mode='markdown')
...
async def handle_ws_set_prompt(event, task_id):
    ...
    await event.edit("请输入新的AI总结提示词：", buttons=[Button.inline("⬅️ 返回AI设置", f"ws_ai_settings:{task_id}")], parse_mode='markdown')
...
async def handle_ws_set_channel(event, task_id):
    ...
    await event.edit("请发送目标频道的 **ID** 或**链接**:", buttons=[Button.inline("⬅️ 返回", f"ws_task:{task_id}")], parse_mode='markdown')

async def handle_ws_change_model(event, task_id):
    buttons = await create_model_selection_buttons_for_task(int(task_id))
    await event.edit("请选择一个AI模型：", buttons=buttons)
