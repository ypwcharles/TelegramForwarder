...
# 为不同的客户端设置不同的会话名称
user_session = os.getenv('USER_SESSION_NAME', 'user')
bot_session = os.getenv('BOT_SESSION_NAME', 'bot')
...
# 创建客户端
user_client = TelegramClient(f'./sessions/{user_session}', api_id, api_hash)
bot_client = TelegramClient(f'./sessions/{bot_session}', api_id, api_hash)
...
async def start_clients():
    # 初始化 DBOperations
    global db_ops, scheduler, chat_updater, web_scrape_scheduler
    db_ops = await DBOperations.create()
    ...
    try:
        ...
        if scheduler:
            scheduler.stop()
        if web_scrape_scheduler:
            web_scrape_scheduler.stop()
        # 停止聊天信息更新器
        if chat_updater:
            chat_updater.stop()
        # 如果 RSS 服务在运行，停止它
